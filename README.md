...
2. Choose the parameter to vary (base frequency, tone spacing, number of tones or test baud), its range and **Step**, and **Repetitions per test**, then select **Start Test**. A run covers every value of the parameter the same number of times, one shuffled pass per repetition, so no value collects more evidence than another and slow drift in the room doesn't always fall on the same values. Tones are computed from the base frequency, the baud and the tone count, so there is no tone spacing to set: gaps are unequal (a Sidon set scaled to the baud), which keeps every pairwise difference distinct so speaker distortion products miss the other tones, and the plan stays inside an octave where it fits so no tone is another's harmonic. 1500 Hz at 25 baud with 4 tones gives 1500/1700/2100/2900 Hz; the fixed control link uses 2800 Hz, which gives 2800/3175/3925/5425 Hz. Each run also writes a **results file** to browser storage — the device that wrote it and its role, every trial's symbol errors and per-symbol, per-tone S/N, every control frame heard with its calibrated tone levels, room tails and FEC corrections, every transmission and the timed log — listed under Saved results and downloadable as JSON for analysis. The panel shows how many test packets the run sends and estimates its length — slower baud takes proportionally longer — warning when it would exceed the 10-minute session limit. The controller proposes settings, waits for the partner's ACK, sends known data and receives raw symbol/bit error counts. On both devices the experiment log records everything that goes over the air: `->` for each frame this device sent, `<-` for each frame it received, and `X` for a frame heard but garbled (header unreadable, signal lost, or CRC failed), with the bytes that were heard. Each line gives the raw data, then its meaning (see Control protocol below); binary test packets are shown as bracketed hex. Repeats, garbled frames and other senders' traffic all appear; only a device's own transmissions heard back by its microphone are left out. Equal error rates do not establish an optimum.
...
Coordination uses fixed 4-FSK at 25 baud on 2800/3175/3925/5425 Hz at amplitude 0.4. Test packets play at the trial's own amplitude, which a run can sweep. A 40 ms symbol is longer than a small room's first reflections (a 10-foot surface echoes at ~18 ms), which is what corrupted 100-baud control messages two feet from the phone in field recordings. The band starts at 2800 Hz because a phone speaker delivers far less below about 2 kHz at a distance: two feet from a phone on a desk, 2900 Hz arrived 14 dB louder than 1500 Hz, and the loud tones' reverberant tails won the quiet tones' symbols. The level is 0.4 because a phone's speaker limiter fades a sustained louder tone across a frame. Control messages are slow: a clean test takes about 40 seconds. CRC, frame-level ACKs and bounded retries protect coordination; this is not a guarantee that the control profile works in every room. A frame sent with an ACK requested is retransmitted after its ACK's air time plus a margin (about 6 seconds), up to 3 times, and never re-measures a test packet: if the partner did not receive it, it says so with `lost` and the controller proposes the same settings again as a new trial. Corrupted control messages (CRC failures) are reported in the experiment log. Quiet guards separate exchanges; the half-second guard before a test packet is its only, unannounced, delay.
...
The results table shows each trial's reception (received, CRC failed or lost), symbol errors and median S/N. Rows are grouped by run, newest at the bottom, under a divider naming the run's sender, start time, trial parameters and (on the controller) what it varies. A partner listening across several controller runs therefore never interleaves their trials: a new sender, or trial numbers starting over, begins a new group. The log's test packet line adds how far symbol tracking moved during the frame, e.g. `drift −0.1 ms`.

**Echo cancellation.** A room reflection arrives a whole number of symbols later carrying a tone the receiver has already decoded, and when it lands on a quieter tone it wins: in a field recording every error was the weakest tone with the loudest tone two symbols (20 ms) earlier. When a frame fails its CRC, the receiver fits one gain per echo delay from that frame's own decisions by least squares, subtracts each decided symbol's echo from later windows, decides again and re-checks the CRC. Gains fitted on a long frame are remembered for short ones, which have too few symbols to fit their own. It costs nothing until a frame fails, and it recovered the control message that ended a two-foot run in the field recording.
...
Sessions end after 10 minutes or the planned tests (at most 200 packets per run), whichever comes first. Stop preserves partial captures; experiment capture is capped at 10 minutes (the Receive tab's recording stays capped at two). Live scoring keeps only the most recent 60 seconds of audio in memory, which spans any single trial. Capture loss aborts optimization and preserves recorded samples for replay. Received acoustic S/N, statistical uncertainty, cross-run aggregation and payload FEC comparisons remain planned. Physical two-device field validation remains necessary.
