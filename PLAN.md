...
- [ ] Replace the early/late symbol-timing gate with a block best fit. Sync acquisition already fits all 16 sync symbols at once and lands to the sample; mid-frame timing then hands off to a decision-directed early/late gate that nudges one offset estimate forward at each isolated transition. Reverberation smears exactly the transition edges that gate depends on. Instead, every N symbols re-fit the window offset over ±⅛ symbol by maximizing the summed score of the last 16 decided tones — the sync's own method, applied continuously — and let the fit also choose where inside each tone the window sits, since at two feet the previous symbol's tail fell from 46% of the window's first 10 ms to 32% of its last. Symbols then re-derive the clock from themselves, as Wozniak's pulse-length cassette decoder did, rather than trusting a single estimate to track. Evidence so far says timing is not the failure (drift under 1 ms over 8-second frames; errors were tone confusion), so this waits on the two-foot retest of the 2800 Hz band and the silence-gap sweep: if frames still fail with large `room carried` percentages, this is the next lever. Prove it on a synthetic reverberant channel where the gate demonstrably drifts and the block fit does not, and on the recorded two-foot frames.
...
- [ ] Read Apple II cassette tapes, tolerating tape speed that warbles. The format is self-clocking: one full cycle per bit, 2000 Hz for a `0` (500 µs) and 1000 Hz for a `1` (1000 µs), after a 770 Hz leader of several seconds and a sync of one half cycle at 2500 Hz followed by one at 2000 Hz; a file ends with a single XOR checksum byte. Decode the way the hardware did — by timing zero crossings rather than filtering for tones — because measuring each cycle's own period is inherently tolerant of playback speed. The part worth building is what the Apple II lacked: instead of a fixed threshold halfway between the two periods, track a running estimate of the current bit periods (seeded from the leader, which is a known 770 Hz and so measures the tape's speed directly) and classify each cycle against a threshold that follows the tape. That should survive several percent of wow and flutter and slow drift across a file, where a fixed threshold fails once speed error approaches 33%. Test it by synthesizing tapes and modulating their playback rate — steady offsets, slow wow, faster flutter, and dropouts — so the decoder's tolerance is a measured number rather than a hope.
